...
interface Job {
  job_id: string // Changed from number to string
  command: string
  preamble: string
  group_name: string
  depends_on: string[]
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={fetchJobs} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            Retry
          </button>
        </div>
      </div>
...
const smartRangeDisplay = (jobIds: string[]): string => {
  const sorted = jobIds.sort((a, b) => Number.parseInt(a) - Number.parseInt(b))
  ...
  if (sorted.length === 1) {
    ...
    const isContinuous = sorted.every((id, i) => i === 0 || Number.parseInt(id) === Number.parseInt(sorted[i - 1]) + 1)
