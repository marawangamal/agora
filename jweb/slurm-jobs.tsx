...
// Static dummy data - no API calls or server-side data fetching
...
// Function to fetch jobs - API call is commented out for development
...
function DependencyGraph({ jobs }: { jobs: Job[] }) {
  ...
  const handleContextAction = (action: string, jobId: string) => {
    ...
    // Note: In static export, these would be client-side only actions
  }
...
export default function Component() {
  ...
  // const [tooltip, setTooltip] = useState<TooltipData>({ job: jobs[0] || DUMMY_JOBS[0], x: 0, y: 0, visible: false })
  ...
  // Client-side only dark mode handling
  useEffect(() => {
  ...
  }, [statusFilter, groupFilter, searchQuery, jobs])

  const uniqueGroups = useMemo(() => {
    return Array.from(new Set(jobs.map((job) => job.group_name).filter(Boolean)))
  }, [jobs])
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">SLURMM Job Dashboard</h1>
          ...
          <TabsContent value="graph" className="mt-6">
            <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
              <CardHeader>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Visual representation of job dependencies. Click groups to view details. Hover for tooltips.
                </p>
              </CardHeader>
              <CardContent>
